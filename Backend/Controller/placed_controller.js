...
const placedController = {

    getAll: async (req, res) => {
        try {
            const [row, fields] = await pool.query("select * from apply_details where status='2' ")
            res.json({data:row})
        } catch (error) {
            console.log(error);
            res.json({
    ...
    update: async (req, res) => {
        try {
            ...
            const [row, fields] = await pool.query("UPDATE `apply_details` SET `status`='2' WHERE `c_id`=?", [c_id])
            res.json({
                data: row
            })
        } catch (error) {
    ...
    delete: async (req, res) => {
        try {
            const { c_id } = req.params
            const [row, fields] = await pool.query("delete from profile where c_id=?", [c_id])
            res.json({
                data: row
            })
        } catch (error) {
...
module.exports = placedController